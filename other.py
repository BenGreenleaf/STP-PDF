...
def recursive_get(sub_assembly_obj, parent_folder_id, destination_folder, existing_parts):
    pending_parts = []
    ...
    while not ref_pos.IsNull:
        ...
        if part_type == 'sldprt' and part_name not in existing_parts:
            ...
            folder_obj = part_file.GetNextFolder(folder_pos)
            part_folder_id = folder_obj.ID
            existing_parts = recursive_get(part_file, part_folder_id, destination_folder, existing_parts)

    if len(pending_parts) != 0 and len(work_in_progress_parts) != 0:
        print(f"Warning: {sub_assembly_obj.Name} Contains work in progress and pending parts which were skipped: {work_in_progress_parts}, {pending_parts}")
    elif len(work_in_progress_parts) != 0:
        print(f"Warning: {sub_assembly_obj.Name} Contains work in progress parts which were skipped: {work_in_progress_parts}")
    elif len(pending_parts) != 0:
...
def main(sub_assembly_name, destination_folder):
    ...
    if sub_assembly_obj:
        existing_parts = recursive_get(sub_assembly_obj, folder.ID, destination_folder, existing_parts)
...
if __name__ == "__main__":
    try:
        ...
        folder_path = config.get('folder_path', r'C:\SSPDM\Projects\P0011-NSIP\Design\SA - Sub Assembly')
